<!--
  rename query to filter
  rename isList to collection?
  columns and filters are the same? speaking in domain logic?
  speaking of: field, operator, value tuples
  bounderies
    tranform column -> filter
-->
<template>
  <div>
    <div class="btn-group mb-3">
      <b-btn class="btn btn-outline-primary" @click="addFilter">Add filter</b-btn>
      <b-btn class="btn btn-outline-primary" @click="applyFilters">Apply</b-btn>
    </div>
    <div class="">
      <div
        v-for="(filter, filterIndex) in filters"
        :key="filterIndex"
        class="form-row"
      >
        <b-form-group class="col-md-4">
          <b-form-select
            v-model="filter.field"
            :options="fieldOptions">
          </b-form-select>
        </b-form-group>
        <b-form-group class="col-md-4">
          <b-form-select
            v-model="filter.operator"
            :options="operators"
            class="col-md-4">
          </b-form-select>
        </b-form-group>
        <b-form-group class="col-md-4">
          <b-form-input
            v-model="filter.value"
            type="text"
          ></b-form-input>
        </b-form-group>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  props: {
    // ordered array of columns to configure
    // expects e.g.: { name: "columnname", state: true }
    columns: {
      type: Array,
      default: () => []
    },
    operators: {
      type: Array,
      default: () => { return ['$gte', '$gt'] }
    }
  },
  data () {
    return {
      // filters use the MongoDB-style query document format
      filters: [{ field: 'credibility', operator: '$gte', value: 1 }]
    }
  },
  computed: {
    fieldOptions () {
      return this.columns.map((column) => {
        return { value: `${column.name}`, text: `${column.name}` }
      })
    }
  },
  methods: {
    // order does not matter
    // all filters will be connected with AND
    addFilter () {
      // this.$emit('toggle', name)
      this.filters.push({})
    },
    applyFilters () {
      this.$emit('apply', this.filters)
    }
  }
}
</script>
